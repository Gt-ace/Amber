...
describe('renderTemplate', () => {
	...
	test('substitutes {{{key}}} raw', () => {
		expect(renderTemplate('<div>{{{html}}}</div>', { html: '<p>hi</p>' })).toBe('<div><p>hi</p></div>');
	});
	...
	test('section over a non-empty array iterates with the element merged over the context', () => {
		...
		const out = renderTemplate(tpl, { items: [{ label: 'Home', href: '/' }, { label: 'About', href: '/about' }] });
	...
	test('nested sections work', () => {
		...
		expect(renderTemplate(tpl, { has_nav: true, nav: [{ label: 'a' }, { label: 'b' }] })).toBe('<ul><li>a</li><li>b</li></ul>');
	...
	test('the content-slot HTML comment passes through and no substitution can forge it', () => {
		expect(renderTemplate('<main>{{x}}<!--amber:content--></main>', { x: '<!--amber:content-->' })).toBe(
			'<main>&lt;!--amber:content--&gt;<!--amber:content--></main>'
		);
	});
});
